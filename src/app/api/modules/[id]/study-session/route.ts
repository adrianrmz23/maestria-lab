...
const durations = new Set<StudyDuration>([5, 10, 15, 30, 45]);
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    if (!payload.durationMinutes || !durations.has(payload.durationMinutes)) return NextResponse.json({ error: "Selecciona 5, 10, 15, 30 o 45 minutos." }, { status: 400 });
