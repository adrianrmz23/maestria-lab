  ...
  return (
    <article>
      <header className="border-b-2 border-ink pb-5">
        ...
        <p className="mt-4 max-w-3xl text-base leading-7 text-muted">{concept.whyItMatters}</p>
      </header>
      ...
      <section className="mt-6 border-l-2 border-moss pl-4 sm:pl-5">
        ...
        <p className="mt-3 max-w-3xl text-sm leading-7 text-ink">{concept.sourceSummary}</p>
      ...
      <section className="mt-8">
        ...
        <div className="flex gap-1 overflow-x-auto border-b border-line scrollbar-none">
          {layers.map((item) => {
            ...
            return (
              <button key={item.id} type="button" onClick={() => setLayer(item.id)} className={`focus-ring relative flex min-h-12 shrink-0 items-center gap-2 px-3 text-xs font-bold sm:px-4 ${selected ? "text-accent" : "text-muted hover:text-ink"}`}>
        ...
        <div className="paper-sheet border-x border-b border-line p-5 sm:p-7 md:p-8">
          ...
          <p className="mt-4 whitespace-pre-line text-[15px] leading-8 text-ink sm:text-base">{concept[layer]}</p>
          <p className="mt-5 border-t border-line pt-3 text-xs leading-5 text-muted">Esta capa es una reformulación pedagógica generada a partir de la fuente; no es una cita textual del documento.</p>
        </div>
      </section>
          ...
          <div className="divide-y divide-line border-b border-line">
            {concept.examples.map((example, index) => (
              <div key={`${example.title}-${index}`} className="grid gap-3 py-5 sm:grid-cols-[130px_minmax(0,1fr)]">
                <div><span className={`meta-font text-[8px] font-bold uppercase ${example.origin === "source" ? "text-moss" : "text-accent"}`}>{example.origin === "source" ? "Ejemplo de fuente" : "Ejemplo generado"}</span><p className="mt-2 text-sm font-bold">{example.title}</p></div>
                <p className="text-sm leading-7 text-muted">{example.content}</p>
              </div>
            ))}
          </div>
        </section>
