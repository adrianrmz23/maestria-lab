...
export const conceptExperienceJsonSchema = {
  ...
  properties: {
    ...
    lab: {
      type: "object",
      additionalProperties: false,
      required: ["type", "title", "objective", "instructions", "difficulty", "expression", "propositions", "operator", "matchingPairs", "sequenceItems", "codeSnippet", "codeQuestion", "codeOptions", "codeAnswerIndex", "parameters", "dataset", "binaryLabels", "matrix", "vector", "sourceRefs"],
      properties: {
        type: { type: "string", enum: ["logic_switch", "truth_table", "matching", "sequence", "code_prediction", "probability_simulator", "statistics_outlier", "ml_threshold", "vector_transform"] },
        title: { type: "string" },
        ...
        dataset: { type: "array", maxItems: 20, items: { type: "number" } },
        binaryLabels: { type: "array", maxItems: 20, items: { type: "integer", enum: [0, 1] } },
        matrix: { type: "array", maxItems: 4, items: { type: "number" } },
        vector: { type: "array", maxItems: 2, items: { type: "number" } },
